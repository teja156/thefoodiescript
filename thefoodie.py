...
try:
    opts, rem = getopt.getopt(argv, 'c', [
        'dishes=', 'categories=', 'ignore-offer-above-limit', 'help', 'city='
    ])
    #print("Options : ",opts)
...
restaurants = scrape_restaurants.go_scraping(city=city,
                                             cfile=cookie_fname,
...
if (len(dishes_list) > 0):
    #print(dishes_list)
    dishes_results = scrape_dishes.go_scraping(cfname=cookie_fname,
    ...
    for dish in dishes_results:
        ...
        sorted_dish_results = sorted(dishes_results[dish],
                                     key=lambda i: i['dish_score'],
                                     reverse=True)
